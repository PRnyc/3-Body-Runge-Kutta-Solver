...
def f(t, y, m):
    ...
    yp[9:12] = -m[1] * (y[0:3] - y[3:6]) / np.linalg.norm(y[0:3] - y[3:6]) ** 3 - m[2] * (
            y[0:3] - y[6:9]) / np.linalg.norm(y[0:3] - y[6:9]) ** 3
    yp[12:15] = -m[0] * (y[3:6] - y[0:3]) / np.linalg.norm(y[3:6] - y[0:3]) ** 3 - m[2] * (
            y[3:6] - y[6:9]) / np.linalg.norm(y[3:6] - y[6:9]) ** 3
    yp[15:18] = -m[0] * (y[6:9] - y[0:3]) / np.linalg.norm(y[6:9] - y[0:3]) ** 3 - m[1] * (
            y[6:9] - y[3:6]) / np.linalg.norm(y[6:9] - y[3:6]) ** 3
    return yp
...
def ODE45(f, tspan, h0, e, y0, s, m):
    ...
    while t[-1] < tspan[1]:
        ...
        k6 = h[i] * f(t[i] + h[i] * 5 / 6,
                      y[i] + k1 * 65 / 432 - k2 * 5 / 16 + k3 * 13 / 16 + k4 * 4 / 27 + k5 * 5 / 144, m)
...
async def get_input_from_backend(websocket, path):
    ...
    h0 = df.loc[0, "h"]
    y0 = [df.loc[0, "x1"], df.loc[0, "y1"], df.loc[0, "z1"], df.loc[0, "x2"], df.loc[0, "y2"], df.loc[0, "z2"],
          df.loc[0, "x3"], df.loc[0, "y3"], df.loc[0, "z3"], df.loc[0, "vx1"], df.loc[0, "vy1"], df.loc[0, "vz1"],
          df.loc[0, "vx2"], df.loc[0, "vy2"], df.loc[0, "vz2"], df.loc[0, "vx3"], df.loc[0, "vy3"], df.loc[0, "vz3"]]
    ...
    try:
        for row in y:
            post.execute(
                '''INSERT INTO Output''' + new_Id + '''(y1,y2,y3,y4,y5,y6,y7,y8,y9,y10,y11,y12,y13,y14,y15,y16,y17,y18) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                row)
